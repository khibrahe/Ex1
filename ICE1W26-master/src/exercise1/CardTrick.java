...
public class CardTrick {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < hand.length; i++) {
            ...
            card.setSuit(Card.SUITS[random.nextInt(3) + 1]);
        ...
        user_card.setValue(card_num);
        user_card.setSuit(suit);

        for (int i = 0; i < hand.length; i++) {
            if (user_card.getValue() == hand[i].getValue()
                    && user_card.getSuit().equalsIgnoreCase(hand[i].getSuit())) {
                printInfo();
                break;
            }
            else {
